...
function Card(props) {
	return (
		<>
			<div className={"p-0.5 rounded-lg bg-gradient-to-r " + (props?.types[0] ? `from-[--type-${props?.types[0].type.name}]` : "") + " " + (props?.types[1] ? `to-[--type-${props?.types[1].type.name}]` : props?.types[0] ? `to-[--type-${props?.types[0].type.name}]` : "")}>
				<div className='w-1/1 rounded-md flex aspect-square flex-col bg-black/[.7] relative'>
					...
					<div className="pokemon-data flex flex-col gap-4 py-4">	
						...
						<Link to={"/" + props.name}><button className="rounded border border-white bg-black/[.4] px-4 py-1 self-center">See more</button></Link>
					</div>
				</div>
			</div>
		</>
